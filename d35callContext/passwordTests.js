...
// const askPassword3 = myExports.askPassword3; 
...
describe("fix function that loses 'this'", function () {
    ...
    it("tests rockstar", function () {
        assert.strictEqual(askPassword3.call(user, "rockstar"), "John logged in");
    });

    it("tests wrong password", function () {
        assert.strictEqual(askPassword3.call(user, "1234"), "John failed to log in");
    });

    it("tests rockstar", function () {
        assert.strictEqual(askPassword3.apply(user, ["rockstar"]), "John logged in");
    });

    it("tests wrong password", function () {
        assert.strictEqual(askPassword3.apply(user, ["1234"]), "John failed to log in");
    });
